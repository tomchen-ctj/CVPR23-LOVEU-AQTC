...
class AffoMiner(LightningModule):
    ...
    def test_step_per_video(self, video, save_dir):
        ...
        while True:
            ...
            state.append(hand_states) 
        states = self.get_hand_states(state)
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        torch.save(states, f'{save_dir}/hands.pth')
        
        
        print(f'{gpu_flag}: {save_dir} have just done! ')
    ...
    def get_hand_states(self, state):
        f = []
        for hand in state:
            if hand.numel() == 0:
                f.append(-1)
...
if __name__ == '__main__':
    affominer = AffoMiner(fps=1, max_side_frames=31)
    num_gpus = 4
    video_path = VideoPath('/home/hy/ssd1/tomchen/loveu2023/dataset', 'train', 'outputs/loveu23', num_gpus, drop_last=False)
    trainer = Trainer(
        devices=num_gpus,
        accelerator='gpu',
